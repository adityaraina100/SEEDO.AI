...
const double MaxBlur         = -70.0;   //more positive = sharper image
...
int main(int argc, char **argv)
{
    ...
    //cv::VideoCapture cap      ("Norton_A.mp4");             //RaspiCam
    //cv::VideoCapture cap(0); // Assuming USB camera is the first device (index 0)
    cv::VideoCapture cap("rtsp://admin:123456@192.168.100.90");   //Movie
    if (!cap.isOpened()) {
                    ...
                    if(FaceCnt>0){
                        ...
                        if(Faces[i].NameProb >= MinFaceThreshold){
                            ...
                            if(Faces[i].rect.height < MinHeightFace){
                                ...
                                std::string postData = "apikey=" + apiKey + "&empName=" + NameFaces[Faces[i].NameIndex];
                                // Construct the cURL command
                                std::string curlCommand = "curl -X POST -d \"" + postData + "\" https://pi360.net/site/api/attendance.php?institute_id=mietjammu";
