...
int main(int argc, char **argv)
{
    ...
    //cv::VideoCapture cap("testfile7.mp4");             //RaspiCam
    //cv::VideoCapture cap(0); // Assuming USB camera is the first device (index 0)
    cv::VideoCapture cap("rtsp://admin:Admin@123@10.253.0.72");   //Movie
    if (!cap.isOpened()) {
                    ...
                    if(FaceCnt>0){
                        ...
                        if(Faces[i].NameProb >= MinFaceThreshold){
                            ...
                            if(Faces[i].rect.height < MinHeightFace){
                                ...
                                std::string postData = "apikey=" + apiKey +"&atype=" + checkout + "&emp_code=" + NameFaces[Faces[i].NameIndex];
                                // Construct the cURL command
                                //std::string curlCommand = "curl -X POST -d \"" + postData + "\" https://reliable-tions-incredible-blank.trycloudflare.com/attendance/attendance.php";
                                 std::string curlCommand = "curl -X POST -d \"" + postData + "\" https://members-ram-consistently-collectors.trycloudflare.com/attendance/attendance.php";
                                // Execute the cURL command

                                    int curlResult = std::system(curlCommand.c_str());
                                    if (curlResult != 0) {
                                        std::cerr << "Failed to make POST request using cURL" << std::endl;
                                    } else {
                                        std::cerr << "Success: POST request using cURL" << std::endl;
                                        // Set the flag to true indicating curl has been triggered
